...
export class TopicServiceProvider {
  ...
  getTopicDetail(news_id){
    return this.http.get('http://59.110.165.55:1007/topic/'+news_id)
      .toPromise().then(res=>{
  ...
  getTopicChildren(t_id){
      return this.http.get('http://59.110.165.55:1007/topic/children/'+t_id)
          .toPromise().then(res=>{
  ...
  //投票
  doVote(id,type){
      return this.http.post('http://59.110.165.55:1007/topic/vote',{id:id,type:type})
          .toPromise().then(res=>{
  ...
  sendTopic(topic,to_id){
      this.http.post('http://59.110.165.55:1007/topic', topic)
          ...
          if (res.json().success) {
              ...
              this.http.post('http://59.110.165.55:1007/topic/add_rply', {id:to_id})
                  .toPromise().then(res => {
                  if (res.json().success) {
                      console.log('回复数+1!');
                  }
